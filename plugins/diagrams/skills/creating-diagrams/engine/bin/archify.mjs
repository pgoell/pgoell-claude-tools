...
// Global --theme <theme.css | theme dir> (pgoell-claude-tools diagrams plugin):
// renderers read ARCHIFY_THEME and inject the brand palette into the artifact.
const cliArgs = process.argv.slice(2);
for (let index = 0; index < cliArgs.length; index += 1) {
  const arg = cliArgs[index];
  if (arg === '--theme' || arg.startsWith('--theme=')) {
    const value = arg === '--theme' ? cliArgs[index + 1] : arg.slice('--theme='.length);
    if (!value || value.startsWith('--')) {
      process.stderr.write('--theme requires a theme.css path or theme directory.\n');
      process.exit(2);
    }
    process.env.ARCHIFY_THEME = path.resolve(value);
    cliArgs.splice(index, arg === '--theme' ? 2 : 1);
    index -= 1;
  }
}
