...
The pgoell-claude-tools `diagrams` plugin removed three marks from this
package because their recorded licenses restrict reuse: Vue.js
(CC-BY-NC-SA-4.0, non-commercial), Jenkins (CC-BY-SA-3.0), and Rust
(CC-BY-SA-4.0). Neither `brand-marks/catalog.json` nor
`renderers/shared/generated-brand-marks.mjs` carries them.
