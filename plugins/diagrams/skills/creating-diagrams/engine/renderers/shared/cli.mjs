...
import { brandThemeStyle } from './brand-theme.mjs';
...
export function writeDiagram({ outPath, template, diagramType, meta, svg, cards, sourceEvidence = null }) {
  ...
  const brand = brandThemeStyle(process.env.ARCHIFY_THEME);
  fs.writeFileSync(outPath, brand ? html.replace('</head>', `${brand}</head>`) : html);
